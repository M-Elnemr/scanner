 ...
 * {@link AnalyticsReportingService}. {@code from}/{@code to} are plain ISO-8601 instants (e.g.
 * {@code 2026-09-04T10:15:30Z}), not calendar dates — Spring's default conversion service binds
 * these to {@link Instant} with no extra annotation, and instant precision is what lets a caller
 * ask for something as narrow as "the last 30 minutes".
 */
...
public class AdminAnalyticsController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/api/v1/admin/analytics/events-over-time")
    public ApiResponse<List<TimeBucketCount>> eventsOverTime(
            @RequestParam(required = false) Instant from, @RequestParam(required = false) Instant to) {
        return ApiResponse.of(analyticsReportingService.eventsOverTime(from, to));
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/api/v1/admin/analytics/most-shared-trips")
    public ApiResponse<List<TripViewCount>> mostSharedTrips(
            @RequestParam(required = false) Instant from,
            @RequestParam(required = false) Instant to,
            @RequestParam(required = false, defaultValue = "10") int limit) {
        return ApiResponse.of(analyticsReportingService.mostSharedTrips(from, to, limit));
    }
}
